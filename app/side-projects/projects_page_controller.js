...
(function () {
  class ProjectsPageCtrl {
    ...
    findData() {
      let currentPage = this.$rootScope.currentPage.substring(
        this.$rootScope.currentPage.lastIndexOf("/") + 1,
        this.$rootScope.currentPage.length
      );

      let wordData = this._.filter(this.PROJECTS, function (o) {
        if (
          o.internalUrl.substring(
            o.internalUrl.lastIndexOf("/") + 1,
            o.internalUrl.length
          ) === currentPage
        ) {
          return o;
        }
    ...
    navigation() {
      ...
      currentPage = this.$rootScope.currentPage.substring(
        this.$rootScope.currentPage.lastIndexOf("/") + 1,
        this.$rootScope.currentPage.length
      );
      ...
      if (!this._.isEmpty(page)) {
        ...
        prevPage = !this._.isEmpty(prevPage)
          ? prevPage
          : this._.filter(this.PROJECTS, function (o, k) {
              if (k === "mq-keyframes") {
                return o;
              }
            });
        ...
        nextPage = !this._.isEmpty(nextPage)
          ? nextPage
          : this._.filter(this.PROJECTS, function (o, k) {
              if (k === "lightning") {
                return o;
  ...
  angular
    .module("portfolioApp.sideProjectsController")
    .controller("ProjectsPageCtrl", [
      ...
      "$window",
      "_",
      function ($rootScope, $scope, $log, PROJECTS, $window, _) {
        return new ProjectsPageCtrl(
          $rootScope,
          $scope,
          $log,
          PROJECTS,
